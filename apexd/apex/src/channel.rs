...
mod tests {
    ...
    fn the_record_path_is_readable_without_root() {
        ...
        // And it goes through the fixture root, or the stop is untestable.
        std::env::set_var("APEX_TRUST_ROOT", "/tmp/apex-record-fixture");
        let p = record_path();
        std::env::remove_var("APEX_TRUST_ROOT");
        assert_eq!(
            p,
            PathBuf::from("/tmp/apex-record-fixture/var/lib/apex/channel/last-update.json")
        );
