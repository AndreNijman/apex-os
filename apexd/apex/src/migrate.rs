...
fn verify(client: &mut Client, f: &Found) -> Result<bool> {
    ...
    //
    // §13.2 ids, and a resource only where the operation declares one:
    // `mcp.request` takes none, which is what stops a caller aiming it
    // anywhere but the endpoint the credential was stored for.
    let (operation, resource, body): (&str, &str, &[u8]) = if f.path.is_empty() {
    ...
    let mut record = CapabilityRecord::new(&f.service, operation, resource);
