...
1. `p4/one-image` first — it moves the semantics. Not valid until a Katana
   build has produced an artifact that was actually inspected: NVIDIA akmod
   present and loadable against the shipped kernel, Mesa still working on
   non-NVIDIA hardware, and all three tags resolving.
2. `fix/gaming-remedy` rebased on top, reconciling the two gaming tests against
   whatever `blockers()` ends up meaning.
