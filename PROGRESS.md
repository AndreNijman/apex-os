...
## Round 23 — 2026-09-13, 03:05–04:00 AWST

All five of round 22's agents were dead when this round opened, 189 minutes
cold, killed together by a usage limit. Every one of them had pushed. Nothing
was messaged; six fresh agents were dispatched off cards.

### Landed

`apex-os` `bb2c278a` → `cd4a799e`, four merges, **zero conflicts** — every
branch had merged the integration tip up into itself first, which is the habit
that makes a landing cheap:

* `84ffb84f` **p1-061** — the unmeasured revocation timing, measured. A
  permission-store change does **not** reach an already-running PipeWire client,
  so `Timing::NextRequest` is the correct statement rather than the cautious
  one. `tests/measure-permission-store-reach.sh` does it on a private session
  bus, a private PipeWire and a private WirePlumber profile that touches no
  hardware — round 1 had thought measuring this meant revoking on Andre's live
  machine, and it does not.
* `7ea639d2` **p2-b round 21** — the installer Tab ring was walked on one page,
  so a focus trap on any other was invisible. Four audited pages now, and two
  more mutants, one of them a safety property.
* `551e9e1a` **p1-053c**, seven commits — and the one worth naming is
  `02e133c0`: `Request::Inject` reads an absolute host path with the daemon's
  own access outside every sandbox, and `inject::handle` refused exactly one
  caller — a managed session — never consulting the origin, because when P1-035
  landed the only imaginable callers were that session and a human at a
  keyboard. `apex-remoted`'s `control()` is a **denylist of one verb**, so a
  paired phone could have had the daemon read `~/.ssh/id_ed25519` into a
  session's inbox and type the path. The module's own threat model names this
  exact attack for a session and gates it there; the remote caller arrived
  later. Gated with `decide`'s own predicate rather than a paraphrase.
* `cd4a799e` **p2-a round 2**, five commits — the ssh backup target against a
  real unprivileged `sshd` on loopback, an S3 provider whose SigV4 signer has
  **five vectors generated by botocore 1.43.71 and none written from memory**,
  the s3 target, and §36's `[identity.agent]` and `[identity.ssh]` enforced
  rather than declared and labelled not-enforced.

`apex-shell` `0c2b7d6` → `bfb24ed`, two merges: p1-061's Privacy page (it threw
twice on every load and nothing static could see it) and p2-b's i18n CI fixes.

### Two branches deliberately not landed

`task/p2-005-device-maturity` is content-landed — the one commit the report's
heuristic flagged was settled with the command the report itself prescribes, and
the diff across the files it touches is empty.

`task/followups-4`'s only commit adds `ROADMAP/state/agents/followups-4.md` to
the apex-os tree. **`ROADMAP/` is not tracked on `roadmap/v2.2`** — the state
directory is snapshotted to `refs/wip/roadmap-state` instead — so landing it
would ship the state directory. The card was extracted with `git show
dd28fd3d:...` and written to disk, and the agent was told to drop the commit.

### Status: the roadmap has no `todo` items left

P2-001 and P2-013 both went `partial` → `done`; P1-059 and P1-060, the last two
`todo`s in the program, went `todo` → `partial`. **90 done / 36 partial / 0 todo
/ 2 blocked of 128.** The two blocked are L-002 and L-003, behind a real TPM
enrol-and-recover cycle on hardware, and they should stay there.

`set-status.py` replaces evidence rather than appending, so P1-061, P2-003 and
P2-004 were re-recorded by reading their existing evidence out of the yaml and
prepending the landing to it.

### Five red tests that are one artifact, and a suite defect underneath them

`cargo test --locked --workspace --no-fail-fast` on the landed tip: **3169
passed / 5 failed** across 50 targets, +37 on the 3132 p2-a measured on its own
branch. All five failures are in `apexd/apex-remoted/tests/relay.rs`, all five
panic at the same line on the same reply:

> a scheduled-job request cannot pair a device: pairing hands a phone standing
> access to this machine's agents, so §7 reserves it for a human at the keyboard

`origin::classify` tests `is_under_user_manager` **first**, and this run's
cgroup was `…/user@1000.service/app.slice/apex-roadmap-resume.service` — the
autoresume timer's own unit. The classifier is working exactly as designed.

Settled structurally rather than by assertion: `git diff --name-only bb2c278a
cd4a799e` touches nothing under `apex-remoted`, `apex-remote-core` or
`origin.rs`. This landing did not cause it.

The defect is in the suite, and it is this repository's dominant CI defect
family wearing its other face — **a test that reports FAILED where it should
report COULD NOT RUN**. `relay.rs` already has a skip for "the daemons did not
come up in this environment" and SKIP-EXTERNAL arms elsewhere; it has no arm for
"this process cannot pair because of where it is running". Handed to
`followups-4`, with the requirement that whatever replaces it must still go red
when pairing is genuinely broken — a skip that swallows a real failure is worse
than the failure. It bears directly on that agent's main job: a GitHub runner's
cgroup is a system service too.

### The gaming check matches its own shell

The ground rule in this file says to run `pgrep -u andre -f "steam|proton"`
before putting load on a machine. Run through a tool that wraps the command in a
shell, **the pattern ends up in the checking process's own cmdline and the check
matches itself** — it answered "gaming active" at 03:14 on a machine with no game
running, and it will always answer that. `pgrep -u andre -af "steam|proton"` and
read what matched. The same trap made a finished `cargo test` look like it was
still running twenty minutes later.

### Seven agents against a ceiling of six

The orchestrator miscounted and started seven. `p2-d` was stopped a few minutes
in and given back. Nothing was lost from the repository — it had created
`task/p2-d-2` and written no commits — but its closing line was *"Three defects
measured in one run"*, it had not yet written them to its card, and a stopped
agent's transcript is not read back. **Those three measurements are gone**, which
is the card contract's own argument made at the orchestrator's expense. Recorded
on `ROADMAP/state/agents/p2-d.md` so round 2 expects to find them again rather
than assuming the ground is clear.

### Dispatched — six agents

`p1-053c` (P1-059/P1-060, the last two todos), `followups-4` (the two CI debt
gates, plus the relay-suite defect above), `p2-b` (P2-003/P2-004 — told
explicitly not to spend the round on the three items that need an ISO, a
listener or a compiled QML extension module), `p2-f` (unblocked by p2-a
finishing; P2-018's automatic entry is not built and it is the criterion's own
verb), `p2-c` (P2-005/006/007 — the spine is P2-006's own finding that a hotspot
cannot work under the shipped firewall policy, which is reachable from the netns
harness with no hardware), and `p1-063` (Andre's lid-closed request, whose
apex-shell half was never written).

Not dispatched, each for a stated reason: `p1-038-hardware` cannot be done
headless and the queue says so in as many words; `later` needs a physical TPM;
`p1-061` says on its own card that nothing was left half-done; `p1-053b` was a
