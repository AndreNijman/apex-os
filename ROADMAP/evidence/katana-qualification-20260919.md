...
| 8 | Wine / XWayland | **PARTIAL PASS** | Wine is not installed, but three real XWayland clients ran under labwc and painted: `steam`, `steamwebhelper` (x2) and `FreeCAD`, all confirmed X11 by socket-peer match. Painted regions: FreeCAD 89.4 %, bbox `(6,6,1808,1074)`. **The specific assertion this row exists for — that an X11 client's titlebar buttons do not sit under the bar mask — was NOT tested**: it needs the window's own geometry against the bar's exclusive zone, and a changed-pixel bounding box is not that. What is established is that XWayland clients run and paint under labwc |
