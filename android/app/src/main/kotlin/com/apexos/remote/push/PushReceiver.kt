...
object PushRegistrar {
     ...
     *
     * Idempotent. A machine that already has a token is left alone — asking
     * again would make the distributor issue a new endpoint and strand the one
     * the machine is using until the new one is delivered and sent on.
     *
    ...
    fun registerIfNeeded(context: Context, machine: PairedMachine): String? {
        ...
        val existing = machine.push?.token
